...
import static com.appops.realmrnd.secure.key.Utils.decodeHexString;
import static com.appops.realmrnd.secure.key.Utils.encodeHexString;
...
public class NonReversibleEncryptionProvider {
    ...
    public static boolean isMatchedData(String plainData, NonReversibleData data) {
        return Arrays.equals(encrypt(plainData.getBytes(), decodeHexString(data.getSalt())),
    ...
    public static NonReversibleData prepareData(String plainPin, NonReversibleData data) {
        ...
        data.setEncryptedData(encodeHexString(encrypt(plainPin.getBytes(), salt)));

        return data;
    }

    private static byte[] encrypt(byte [] plainData, byte[] salt) {
        try {
